...
<template>
    ...
    <div class="z-10  text-white w-full h-full ">
        ...
        <div v-if="orders.length > 0" class="mx-auto place-self-center  py-10 rounded-md   md:w-11/12 lg:w-2/5  w-full   items-center justify-items-center  ">

            <h1 class="text-4xl lg:text-5xl text-center  font-bold my-24">Список заказов</h1>
                        ...
                        <tr v-if="orders" v-for="order in orders" class="border-b ">
                            ...
                            <td class=" flex justify-center py-3">
                                ...
                                <Link v-if="order && order.status === 'Новый'" @click.prevent="orderComplete(order.id)">
                                    <button class="bg-green-700 py-1 px-2 hover:bg-white hover:text-green-700  rounded-lg border border-green-800 text-lg transform duration-500 ease-out"> <i class="ri-check-line"></i>
                                    </button>
                                </Link>
                                <Link v-if="order && order.id" class="mx-1" :href="route('order.check',{id: order.id})">
                                    <button class="bg-yellow-500 py-1 px-2 rounded-lg border border-yellow-500 text-lg hover:text-yellow-500 hover:bg-gray-200
                         hover:border-white transform duration-500 ease-out"> <i class="ri-eye-fill"></i>
                                ...
                                <div  @click.prevent="deleteOrder(order.id)"  v-if="order && order.id" >
                                    <button class="bg-red-600 py-1 px-2 rounded-lg border border-red-800 text-lg hover:text-red-500 hover:bg-gray-200
        ...
        <div v-else class="flex-col flex space-y-12 text-2xl md:text-4xl lg:text-5xl font-lobster text-center py-44 font-medium">
            <span>У вас нет заказов</span>
            <div class="lg:w-2/5 w-2/3 place-self-center md:w-3/4 h-1 rounded-md bg-white"></div>
